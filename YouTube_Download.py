...
screen.wm_iconbitmap(r"C:\Users\sahil\Desktop\apps\Projects\YoutubeVideo_downloader\youtube_downloader.ico")
...
def done():
    ...
    done_btn.wait_visibility()
    youtube = YouTube(link_field.get())
    progress = ttk.Progressbar(screen,orient=HORIZONTAL,length=100, mode='determinate')
    canvas.create_window(300, 350, window=progress)
    if choice == 'video':
        video(youtube)
    elif choice == 'audio':
        audio(youtube)
